...
import { deleteEventsBanner } from '@/actions/banner-actions/events-banner'

interface EventsBanner {
  id: string
  title: string
  subtitle: string
  image: string
...
interface EventsBannerTableProps {
  banners: EventsBanner[]
}

export function EventsBannerTable({ banners }: EventsBannerTableProps) {
  ...
  const handleDelete = async () => {
    ...
    try {
      setIsDeleting(true)
      const result = await deleteEventsBanner(selectedId)
  ...
  if (banners.length === 0) {
    return (
      <div className="text-center py-12">
        <p className="text-muted-foreground">No events banners found. Create one to get started.</p>
      </div>
    )
  }
                ...
                <td className="py-3 px-4 text-right">
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem
                        onClick={() => router.push(`/dashboard/banners/events/${banner.id}`)}
                      >
                        View
                      </DropdownMenuItem>
                      <DropdownMenuItem
                        onClick={() => router.push(`/dashboard/banners/events/edit/${banner.id}`)}
                      >
                        Edit
                      </DropdownMenuItem>
                      <DropdownMenuItem
