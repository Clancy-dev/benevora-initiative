...
import { deleteAboutBanner, getAboutBanner } from '@/actions/banner-actions/about-banner'
import toast from 'react-hot-toast'

interface AboutBannerDetailPageProps {
  params: Promise<{ id: string }>
}

export default function AboutBannerDetailPage({ params }: AboutBannerDetailPageProps) {
    ...
    async function fetchBanner() {
      try {
        ...
        if (mounted) {
          ...
          const result = await getAboutBanner(resolvedParams.id)

          if (!result.success) {
            toast.error('Banner not found')
            router.push('/dashboard/banners/about')
            return
          }
  ...
  const handleDelete = async () => {
    ...
    try {
      setIsDeleting(true)
      const result = await deleteAboutBanner(id)
  ...
  return (
    <div className="container mx-auto p-8">
      ...
      <Card className="max-w-2xl mx-auto">
        <CardHeader>
          <CardTitle>About Banner Details</CardTitle>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="flex gap-3 justify-end pt-4">
            <Link href={`/dashboard/banners/about/edit/${banner.id}`}>
              <Button>Edit</Button>
            </Link>
            <Button
              variant="destructive"
