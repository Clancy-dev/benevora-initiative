...
import { getDeletedMembershipBanners, permanentDeleteMembershipBanner, restoreMembershipBanner } from '@/actions/banner-actions/membership'
...
export default function TrashPage() {
  ...
  async function fetchDeletedBanners() {
    try {
      setIsLoading(true)
      const result = await getDeletedMembershipBanners()
  ...
  const handleRestore = async () => {
    ...
    try {
      ...
      const result = await restoreMembershipBanner(selectedId)
  ...
  const handlePermanentDelete = async () => {
    ...
    try {
      setIsProcessing(true)
      const result = await permanentDeleteMembershipBanner(selectedId)
    ...
    <div className="container mx-auto p-8">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-muted-foreground mt-2">
              Manage deleted membership banners
            </p>
          </div>
        </div>
      </div>
      ...
      <AlertDialog open={showDialog} onOpenChange={setShowDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {action === 'restore' ? 'Restore Membership Banner' : 'Permanently Delete Membership Banner'}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {action === 'restore'
