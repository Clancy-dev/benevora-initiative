...
import { getDeletedContactBanners, permanentDeleteContactBanner, restoreContactBanner } from '@/actions/banner-actions/contact-banner'
...
export default function TrashPage() {
  ...
  async function fetchDeletedBanners() {
    try {
      setIsLoading(true)
      const result = await getDeletedContactBanners()
  ...
  const handleRestore = async () => {
    ...
    try {
      ...
      const result = await restoreContactBanner(selectedId)
  ...
  const handlePermanentDelete = async () => {
    ...
    try {
      setIsProcessing(true)
      const result = await permanentDeleteContactBanner(selectedId)
    ...
    <div className="container mx-auto p-8">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-muted-foreground mt-2">
              Manage deleted contact banners
            </p>
          </div>
        </div>
      </div>
      ...
      <AlertDialog open={showDialog} onOpenChange={setShowDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {action === 'restore' ? 'Restore Contact Banner' : 'Permanently Delete Contact Banner'}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {action === 'restore'
