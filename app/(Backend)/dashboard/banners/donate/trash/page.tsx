...
import { getDeletedDonateBanners, permanentDeleteDonateBanner, restoreDonateBanner } from '@/actions/banner-actions/donate-banner'
...
export default function TrashPage() {
  ...
  async function fetchDeletedBanners() {
    try {
      setIsLoading(true)
      const result = await getDeletedDonateBanners()
  ...
  const handleRestore = async () => {
    ...
    try {
      ...
      const result = await restoreDonateBanner(selectedId)
  ...
  const handlePermanentDelete = async () => {
    ...
    try {
      setIsProcessing(true)
      const result = await permanentDeleteDonateBanner(selectedId)
    ...
    <div className="container mx-auto p-8">
      ...
      <AlertDialog open={showDialog} onOpenChange={setShowDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {action === 'restore' ? 'Restore Donate Banner' : 'Permanently Delete Donate Banner'}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {action === 'restore'
