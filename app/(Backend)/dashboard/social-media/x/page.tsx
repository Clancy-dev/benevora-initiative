...
export default async function XPage() {
  const result = await getAllXLinks();
  const links = result.success ? result.data ?? [] : [];

  const hasXLink = links.length > 0;
        ...
        <div className="space-y-3 flex flex-col gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground mt-2">
              Manage your X (Twitter) profile links
            </p>
          </div>
        </div>

        <div className="flex gap-2">
          <Link href="/dashboard/social-media/x/trash">
            <Button
              variant="outline"
              size="sm"
          ...
          {hasXLink ? (
            ...
            <Link href="/dashboard/social-media/x/create">
              <Button
                size="sm"
