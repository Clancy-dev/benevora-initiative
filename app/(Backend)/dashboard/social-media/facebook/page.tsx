...
export default async function FacebookPage() {
  const result = await getAllFacebookLinks();
  const links = result.success ? result.data ?? [] : [];

  const hasFacebookLink = links.length > 0;
  ...
  return (
    <div className="space-y-8 p-8">
        ...
        <div className="space-y-3 flex flex-col gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground mt-2">
              Manage your Facebook page links
            </p>
          </div>
        </div>

        <div className="flex gap-2">
          <Link href="/dashboard/social-media/facebook/trash">
            <Button
              variant="outline"
              size="sm"
          ...
          {hasFacebookLink ? (
            ...
            <Link href="/dashboard/social-media/facebook/create">
              <Button
                size="sm"
      ...
      <FacebookTable data={links as any} />
    </div>
  );
}