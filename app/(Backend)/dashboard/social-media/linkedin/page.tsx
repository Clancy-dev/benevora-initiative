...
export default async function LinkedinPage() {
  const result = await getAllLinkedinLinks();
  const links = result.success ? result.data ?? [] : [];

  const hasLinkedinLink = links.length > 0;
  ...
  return (
    <div className="space-y-8 p-8">
        ...
        <div className="space-y-3 flex flex-col gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground mt-2">
              Manage your LinkedIn profile links
            </p>
          </div>
        </div>

        <div className="flex gap-2">
          <Link href="/dashboard/social-media/linkedin/trash">
            <Button
              variant="outline"
              size="sm"
          ...
          {hasLinkedinLink ? (
            ...
            <Link href="/dashboard/social-media/linkedin/create">
              <Button
                size="sm"
      ...
      <LinkedinTable data={links as any} />
    </div>
  );
}